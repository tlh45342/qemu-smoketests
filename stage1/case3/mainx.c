...
int main(void)
{
    ...
    // Bounding box around where we expect the text to be
    uint32_t len = 0;
    for (const char *p = text; *p; ++p) len++;

    uint32_t x0 = tx;
    uint32_t y0 = ty;
    uint32_t x1 = tx + len * FONT_W;
    uint32_t y1 = ty + FONT_H;

    // Rectangle outline in yellow
    for (uint32_t x = x0; x <= x1 && x < WIDTH; ++x) {
        put_pixel(x, y0, YELLOW);
        put_pixel(x, y1, YELLOW);
    }
    for (uint32_t y = y0; y <= y1 && y < HEIGHT; ++y) {
        put_pixel(x0, y, YELLOW);
        put_pixel(x1, y, YELLOW);
    }
